...
//
// Solidity: function proveL1Native((address,bytes32,bytes32,bytes32) _args, bytes _rlpEncodedL1Header, bytes[] _l1StorageProof, bytes _rlpEncodedContractAccount, bytes[] _l1AccountProof) view returns(uint256 chainID, address storingContract, uint256 blockNumber, bytes32 storageSlot, bytes32 storageValue)
func (_NativeProver *NativeProverCaller) ProveL1Native(opts *bind.CallOpts, _args ProveL1ScalarArgs, _rlpEncodedL1Header []byte, _l1StorageProof [][]byte, _rlpEncodedContractAccount []byte, _l1AccountProof [][]byte) (struct {
	...
	outstruct.StorageSlot = *abi.ConvertType(out[3], new([32]byte)).(*[32]byte)
	outstruct.StorageValue = *abi.ConvertType(out[4], new([32]byte)).(*[32]byte)
