...
contract NativeProver is INativeProver, IProverHelper {
    ...
    mapping(uint256 => L2Configuration) public l2ChainConfigurations; // Mapping of counterparty chainIDs to their configurations
    ...
    modifier validRLPEncodeBlock(
        bytes memory _rlpEncodedHeader,
        bytes32 _expectedBlockHash
    ) {
    ...
    function _setInitialChainConfiguration(
        uint256 _chainID,
    ...
    **/
    function updateL2ChainConfiguration(
        ...
        require(_proveL2Configuration(
            _chainID,
            _config,
            _l1StorageProof,
            _rlpEncodedRegistryAccountData,
            _l1RegistryProof,
            _l1WorldStateRoot), "Invalid L2Configuration proof");
        l2ChainConfigurations[_chainID] = _config;
    }
    ...
    **/
    function updateL1ChainConfiguration(
        ...
        require(_proveL1Configuration(
            _config,
            _l1StorageProof,
            _rlpEncodedRegistryAccountData,
            _l1RegistryProof,
            _l1WorldStateRoot), "Invalid L1Configuration proof");
    ...
    **/
    function _proveL2Configuration(
        ...
        bytes memory registryStorageRoot = RLPReader.readBytes(
            RLPReader.readList(_rlpEncodedRegistryAccountData)[2]
        );
        ...
        bytes32 configHashStorageSlot = bytes32(
            (uint256(keccak256(abi.encode(_chainID))) + L1_CONFIGURATION.settlementRegistryL2ConfigMappingSlot)
        );
        _proveStorageBytes32(
            abi.encodePacked(configHashStorageSlot),
            configHash,
            _l1StorageProof,
    ...
    **/
    function _proveL1Configuration(
        ...
        bytes32 configHash = keccak256(abi.encode(_config));
        bytes32 configHashStorageSlot = bytes32(
            (uint256(keccak256(abi.encode(CHAIN_ID))) + L1_CONFIGURATION.settlementRegistryL1ConfigMappingSlot)
        );
        _proveStorageBytes32(
            abi.encodePacked(configHashStorageSlot),
            configHash,
    ...
    validRLPEncodeBlock(_rlpEncodedL1Header, IL1Block(L1_CONFIGURATION.blockHashOracle).hash())
    {
        ...
        BlockProof memory blockProof = BlockProof({
            blockNumber: _bytesToUint(
                RLPReader.readBytes(RLPReader.readList(_rlpEncodedL1Header)[8])
            ),
        ...
        if (
            existingBlockProof.blockNumber + L1_CONFIGURATION.settlementBlocksDelay <
    ...
    function proveSettledState(
        ...
        proveSettledState(conf, _l2WorldStateRoot, _rlpEncodedL2Header, _l1WorldStateRoot, _proof), "Invalid settled state proof");

        // Update proven state if newer block
        BlockProof memory existingBlockProof = provenStates[_chainID];
        BlockProof memory blockProof = BlockProof({
            blockNumber: _bytesToUint(
                RLPReader.readBytes(RLPReader.readList(_rlpEncodedL2Header)[8])
            ),
    ...
    returns (uint256 chainID, address storingContract, bytes32 storageValue) {
        ...
        bytes memory storageRoot = RLPReader.readBytes(
            RLPReader.readList(_rlpEncodedContractState)[2]
        );
