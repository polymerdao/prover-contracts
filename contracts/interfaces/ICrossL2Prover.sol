...
interface ICrossL2Prover is IClientUpdates {
    ...
    function validateReceipt(bytes calldata proof)
        external
        view
        returns (bytes memory srcChainId, bytes calldata receiptRLP);
    ...
    function validateEvent(uint256 logIndex, bytes calldata proof)
        external
        view
        returns (bytes memory chainId, address emittingContract, bytes[] calldata topics, bytes calldata unindexedData);
