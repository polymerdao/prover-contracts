...
/**
 * @title DeployOPStackBedrockProverCreate2Script
 * @notice Deploys the OPStackBedrockProver contract with CREATE2 for deterministic addresses
 * @dev This script deploys the OPStackBedrockProver contract using CREATE2 with a fixed salt.
 ...
 */
contract DeployOPStackBedrockProverCreate2Script is Script {
    function run() external returns (address opStackBedrockProver) {
        ...
        bytes32 salt = keccak256(abi.encodePacked("polymer-opstack-bedrock-prover"));
        ...
        OPStackBedrockProver proverContract = new OPStackBedrockProver{salt: salt}();
        
        console.log("Deployed OPStackBedrockProver using CREATE2 at address:", address(proverContract));
