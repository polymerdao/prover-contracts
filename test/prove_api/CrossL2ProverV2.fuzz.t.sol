...
contract CrossL2ProverV2FuzzTest is SigningBase {
    ...
    function testFuzz_validateEvent_arbitraryNeverValidates(bytes calldata proof) public {
        vm.expectRevert();
        crossProverV2.validateEvent(proof);
    }
    ...
    // Forgery resistance at the crypto boundary: the sequencer signs over (stateRoot, peptideHeight).
    // Starting from a genuine proof and tampering ONLY those signed fields keeps r/s/v well-formed, so
    // the call reaches the signature check and ECDSA recovers a different signer -- it must revert with
    // exactly InvalidSequencerSignature. Unlike arbitrary bytes (which die on early slicing), this
    // probes the actual signature check, and asserting the specific error proves *why* it was rejected.
    function testFuzz_validateEvent_tamperedSignedFieldRejected(uint256 bytePos, uint8 mask) public {
        bytes memory m = bytes.concat(validProof);
        m[_signedFieldByte(bytePos)] ^= bytes1(uint8(bound(mask, 1, type(uint8).max))); // guarantee a flip

        vm.expectRevert(SequencerSignatureVerifierV2.InvalidSequencerSignature.selector);
        crossProverV2.validateEvent(m);
    }

    // ---------------------------------------------------------------- validateSolLogs

    function testFuzz_validateSolLogs_arbitraryNeverValidates(bytes calldata proof) public {
        vm.expectRevert();
        crossProverV2.validateSolLogs(proof);
    }

    // Same signed fields gate validateSolLogs; tampering them is rejected at the signature check too.
    function testFuzz_validateSolLogs_tamperedSignedFieldRejected(uint256 bytePos, uint8 mask) public {
        bytes memory m = bytes.concat(validSolProof);
        m[_signedFieldByte(bytePos)] ^= bytes1(uint8(bound(mask, 1, type(uint8).max)));

        vm.expectRevert(SequencerSignatureVerifierV2.InvalidSequencerSignature.selector);
        crossProverV2.validateSolLogs(m);
    }
    ...
    function testFuzz_verifyMembership_arbitraryNeverProves(
        ...
        vm.expectRevert(); // InvalidProofRoot or out-of-bounds
        crossProverV2.verifyMembership(root, key, value, proof);
    }
    ...
    // Maps a fuzzed index onto a byte the sequencer actually signs: stateRoot [0:32] then
    // peptideHeight [101:109] (layout per CrossL2ProverV2.validateEvent). r/s/v are left untouched.
    function _signedFieldByte(uint256 bytePos) internal pure returns (uint256) {
        uint256 i = bound(bytePos, 0, 39); // 32 state-root bytes + 8 peptide-height bytes
        return i < 32 ? i : 101 + (i - 32);
    }
}
